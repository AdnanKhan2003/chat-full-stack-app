...
const ChatMessageInput = ({
  ...
  const handleChangeInput = (val) => {
    ...
    typingTimeoutRef.current = setTimeout(() => {
      ...
      const timeDiff = timeNow - timeWhenTypingStart;
      if (timeDiff >= timer && isCurrentUsertyping) {
  ...
  return (
    <>
      {isChatPartnertyping && activeChat && <TypingIndicator />}
      <form
        onSubmit={(e) => {
          e.preventDefault();
          onSend(e);
      ...
      >
        <div>
          <input
            ...
            onChange={(e) => handleChangeInput(e.target.value, activeChat)}
            onBlur={() => {
              socket.emit("stop typing", activeChat._id);
              setIsCurrentUsertyping(false);
            }}
